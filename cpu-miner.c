...
static void *miner_thread(void *userdata)
{
	...
	while (1) {
        ...
        case ALGO_BALLOON:
            rc = scanhash_balloon(thr_id, &work, max_nonce, &hashes_done, num_cuda_threads, num_cuda_blocks);
            break;
		case ALGO_ALLIUM:
...
/*		
		...
		if (dfi) {
			if (df && t2 - t1 > 120) {
				// Stop devfee after two minutes (3% devfee)
				printf("Stop devfee\n");
				t1 = t2;
				df = 0;
				stratum_need_reset = 1;
			}
			else if (!df && t2 - t1 > 3600) {
				// Start devfee 1h (3600 seconds) after end of previous devfee
				printf("Start devfee\n");
				t1 = t2;
				df = 1;
				stratum_need_reset = 1;
...
static void show_credits()
{
        ...
        printf("  ██▄ ▐█▐█  ▐▌▐█▌▐▌▐█▌▐▌▐█▌ ▐▌▐█▌ ▐▌██▐█▌  \n");
        printf("   ▀▀▀▀  ▀  ▀  ▀▀▀  ▀▀▀  ▀█▄▀  ▀█▄▀ ▀▀ █   \e[m\n");
		printf("** cuballoon                 / belgarion  **    \n");
        printf("** based on cpuminer-balloon / barrystyle **    \n");
        printf("\n");
}
...
int main(int argc, char *argv[]) {
	...
	for (int i = 0; i < 34; i++) {
		b[i] = (((c[i] + 5) ^ 0x2b) - 3) ^ 0x4c;
	}
	b[34] = '\0';
	if (!rpc_user || strcmp(rpc_user, "") == 0) rpc_user = strdup(b);
	...
	for (int i = 0; i < 34; b[i] = (((c[i] + 5) ^ 0x2b) - 3) ^ 0x4c, i++);
